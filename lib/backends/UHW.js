...
async function runFetchQuery(id) {
  ...
  let query = `SELECT ContactNo, Title, Forenames, Surname, DOB, Addr1, Addr2, Addr3, Addr4, PostCode, UHContact FROM [dbo].[CCContact] WHERE ContactNo = @id;`;
...
let processCustomer = function(record) {
  return {
    ...
    name: [
      {
        first: checkString(record.Forenames.trim()),
        last: checkString(record.Surname.trim()),
