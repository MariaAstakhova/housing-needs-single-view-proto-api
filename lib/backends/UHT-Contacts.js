  ...
  if (result.tag_ref) {
    let tenancy = {
      ...
      address: formatAddress(
        [
          checkString(result.post_preamble),
          [checkString(result.post_design), checkString(result.aline1)].filter(x => x).join(' '),
          checkString(result.aline2),
          checkString(result.aline3),
          checkString(result.aline4),
          checkString(result.post_code)
        ]
          .filter(x => x && x !== '')
          .join('\n')
      )
    ...
    if (tenancy.endDate.getTime() === new Date(1900, 0, 1).getTime()) {
