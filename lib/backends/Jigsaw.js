...
let fetchCustomerNotes = async function(id, token) {
  console.log('Fetching jigsaw notes...');
  console.time('Jigsaw fetch notes');
  const result = await doJigsawGetRequest(notesUrl(id), null, token);
  console.timeEnd('Jigsaw fetch notes');
  return result;
};
