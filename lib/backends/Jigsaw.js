...
const { nameCase, formatAddress } = require('../Utils');
  ...
  if (results.length == 1 && results[0].id == 0) {
    ...
    return results.map(record => {
      return {
        ...
        dob: moment(record.doB).format('DD/MM/YYYY'),
...
const processAccomPlacements = function(result) {
  ...
  if (result.isCurrentlyInPlacement) {
    ...
    customer.housingNeeds.currentPlacement.startDate = moment(
      curr.startDate
    ).format('DD/MM/YYYY');
    customer.housingNeeds.currentPlacement.endDate = moment(
      curr.endDate
    ).format('DD/MM/YYYY');
  }
...
const processCustomer = function(result) {
  ...
  customer.address = [formatAddress(info.addressString)];
  customer.dob = [moment(info.dateOfBirth).format('DD/MM/YYYY')];
