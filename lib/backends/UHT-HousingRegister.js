...
const { formatPhone, formatAddress, checkString } = require('../Utils');
...
let processCustomer = function(result) {
  return {
    systemIds: {
      uhtHousingRegister: [result.app_ref]
    },
    name: [
      {
        first: checkString(result.forename),
        last: checkString(result.surname),
    ...
    address: [formatAddress(result.m_address), formatAddress(result.corr_addr)],
