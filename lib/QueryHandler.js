...
const QueryHandler = {
  searchCustomers: async (query, systems) => {
    let results = [];
    for (const system of systems) {
      try {
        const result = await backends[system].customerSearch(query);
  ...
  saveCustomer: async input => {
    ...
    const customer = await PostgresDb.one(insertCustQuery, [
      getParam("firstName", input.customers, name => name ? name.toUpperCase() : null),
      getParam("lastName", input.customers, name => name ? name.toUpperCase() : null),
      getParam("address", input.customers, address => address ? address.toUpperCase() : null),
      getParam("dob", input.customers, dob =>
        dob ? moment(dob, "DD/MM/YYYY").format() : null
      ),
      getParam("nino", input.customers, nino => nino ? nino.toUpperCase() : null)
    ]);
  ...
  fetchCustomer: async id => {
    ...
    let requests = links.map(async link => {
      try {
        return await backends[link.name].fetchCustomer(link.remote_id);
      } catch (err) {
    ...
    let customer = mergeResponses(result);
  ...
  fetchCustomerNotes: async id => {
    ...
    for (const link of links) {
      try {
        const result = await backends[link.name].fetchCustomerNotes(
          link.remote_id
        );
        results = results.concat(result);
      } catch (err) {
  ...
  fetchCustomerDocuments: async id => {
    ...
    for (const link of links) {
      try {
        const result = await backends[link.name].fetchCustomerDocuments(
          link.remote_id
        );
        results = results.concat(result);
      } catch (err) {
