...
let mergeAddresses = function(addresses) {
  ...
  return Object.values(reduced).map(arr => {
    try {
      ...
      console.error(err);
      console.error(JSON.stringify(addresses));
      console.error(JSON.stringify(reduced));
      return null;
    }
  });
};
...
let cleanupData = function(record) {
  ...
  if (
    record.address &&
    record.address === '10 Elmbridge Walk, Blackstone Estate, London, E8 3HA'
  ) {
    record.address = null;
  }
  return record;
